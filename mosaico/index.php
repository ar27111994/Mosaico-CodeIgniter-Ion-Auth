...
<html>
  <head>
    ...
    <link rel="stylesheet" href="<?php echo base_url(); ?>mosaico/dist/mosaico-material.min.css" />
    ...
    <script>
        ...
        var initialEdits = [];
        <?php if( !empty( json_decode($templates) ) ) { ?>
        var templates = <?php echo $templates; ?>;
        if ($.isArray(templates) &&  templates.length) {
            for (var i = 0; i < templates.length; i++) {
              md = (templates[i].template_metadata);
              if (typeof md == 'string') {
                initialEdits.push(JSON.parse(md));
              } else {
                console.log("Ignoring saved key", templates[i], "type", typeof md, md);
              }
            }
        ...
        viewModel.newEdit = function(shorttmplname) {
            ...
            var d = new Date();
            var rnd = '<?php echo uniqid('', true); ?>';
            var name = prompt("Please enter a template name:", "<?php echo uniqid('', true); ?>");
            if (name == null || name == "") {
                name = "<?php echo uniqid('', true); ?>";
            }
            var template = '<?php echo base_url(); ?>mosaico/templates/'+shorttmplname+'/template-'+shorttmplname+'.html';
            viewModel.edits.unshift({ created: Date.now(), key: rnd, name: shorttmplname, template: template });
            document.location = '<?php echo base_url(); ?>Email_templates/editor/?template=templates/'+shorttmplname+'/template-'+shorttmplname+'.html&name='+name+'#'+rnd;
            // { data: 'AAAA-MM-GG', key: 'ABCDE' }
            // viewModel.edits.push(template);
        };
        viewModel.renameEdit = function(index) {
            var newName = window.prompt("Please enter a template name:", viewModel.edits()[index].name);
            if (newName) {
              var newItem = JSON.parse(ko.toJSON(viewModel.edits()[index]));
              newItem.name = newName;
              var server = '<?php echo base_url(); ?>';
              $.ajax({
                        url:  server + 'Email_templates/save_template/', // Path to load.php
                        type: 'post',
                        dataType: 'JSON',
                        data: { hash: newItem.key, name: newName, metadata: ko.toJSON(newItem)},
        ...
        viewModel.deleteEdit = function(index) {
            ...
            if (confirm) {
              var server = '<?php echo base_url(); ?>';
              $.ajax({
                        url:  server + 'Email_templates/delete_template/' + viewModel.edits()[index].key, // Path to load.php
                        type: 'get',
                        dataType: 'JSON',
                        success: function(data){
...
// $('<hr/>').appendTo($('body'));
  ...
  <div style="background-color: #d2cbb1; padding: 10px;">
    <table class="logoWrapper" valign="bottom" align="center"><tr><td valign="bottom"><img class="logoImage" alt="Mosaico.io" style="display: block;" src="<?php echo base_url(); ?>mosaico/dist/img/mosaicologo.png" /><div class="logoContainer"></div></td><td class="byTable" valign="bottom"><a href="http://www.voxmail.it"><img src="<?php echo base_url(); ?>mosaico/dist/img/byvoxmail.png" alt="by VOXmail" /></a></td></tr></table>
  ...
  <div class="disclaimer" style="position: absolute; top: 10px; right: 10px; width: 140px; padding: .5em; background-color: #900000; color: white; border: 2px dashed #d2cbb1">WARNING: experimental beta version, use with care!</div>
    ...
    <!-- ko ifnot: $root.showSaved --><span>Previously saved Email Templates <a class="resumeButton" href="#" data-bind="click: $root.showSaved.bind(undefined, true);"><i class="fa fa-plus-square"></i> Show</a></span><!-- /ko -->
    <!-- ko if: $root.showSaved -->
    <table id="savedTable" align="center" cellspacing="0" cellpadding="8" style="padding: 5px; ">
    <caption>Previously saved Email Templates <a href="#" class="resumeButton" data-bind="click: $root.showSaved.bind(undefined, false);"><i class="fa fa-minus-square"></i> Hide</a></caption>
      <thead><tr>
        <th>Id</th><th>Name</th><th>Created</th><th>Last changed</th><th>Operations</th>
      </tr></thead>
    <tbody data-bind="foreach: edits">
      <tr>
        <td align="left"><a href="#" data-bind="attr: { href: '<?php echo base_url(); ?>Email_templates/editor/?template='+template+'&name='+name+'#'+key }"><code>#<span data-bind="text: key">key</span></code></a></td>
        <td style="font-weight: bold" align="left"><a href="#" data-bind="click: $root.renameEdit.bind(undefined, $index())"><span data-bind="text: name">versamix</span></a></td>
        ...
        <td>
        <a class="operationButton" href="#" data-bind="attr: { href: '<?php echo base_url(); ?>Email_templates/editor/?template='+template+'&name='+name+'#'+key }" title="edit"><i class="fa fa-pencil"></i></a>
    ...
    <div data-bind="foreach: templates">
      <div class="template template-xx" style="" data-bind="attr: { class: 'template template-'+name }">
        ...
        <a href="#" data-bind="click: $root.newEdit.bind(undefined, name), attr: { href: '<?php echo base_url(); ?>Email_templates/editor/?template=templates/'+name+'/template-'+name+'.html' }">
          <img src width="100%" alt="xx" data-bind="attr: { src: '<?php echo base_url(); ?>mosaico/templates/'+name+'/edres/_full.png' }">
        </a>
      </div>
    </div>
    </div>
    ...
    <div class="footer" style="background-color: #3f3d33; color: #d2cbb1; padding: 1em">
      Void Labs Snc 2015速 - All rights reserved - P.IVA 02137700395
    </div>
  </body>
</html>
